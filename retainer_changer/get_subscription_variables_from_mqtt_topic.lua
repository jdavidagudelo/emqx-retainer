...
local function get_mqtt_topic_kind(topic)
    ...
    local last_value_patterns = {regex_last_value, regex_last_value_variable_wild_cards, regex_last_value_variable_device_wild_cards,
...
local function can_view_value_device_function(permissions_type, owner_id, device_label, token)
    ...
    if type(device_id) == 'string' then
        if permissions_type == 'device' and type(token) == 'string' then
            can_view_value_device = redis.call('SISMEMBER', 'reactor_devices_with_permissions/view_value/' .. token, device_id)
        end
    end
...
if type(token)  == 'string' then
    ...
    if type(owner_id) == 'string' then
        if device_label ~= "*" and variable_label ~= "*" then
            local can_view_value_device = can_view_value_device_function(permissions_type, owner_id, device_label, token)
            if can_view_value_device then
                local result = {}
                local variable_id = redis.call('HGET', 'reactor_variables/' .. owner_id, '/' .. device_label .. '/' .. variable_label)
